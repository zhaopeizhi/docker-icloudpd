...
Notify(){
   ...
   if [ "${notification_type}" = "Prowl" ]; then
      ...
      notification_result="$(curl --silent --output /dev/null --write-out "%{http_code}" --data-ascii "{\"touser\":\"${wecom_touser}\",\"msgtype\":\"text\",\"agentid\":\"${wecom_agentid}\",\"text\":{\"content\":\"${notification_title}, ${syn_end_time}, ${wecom_text}\"},\"safe\":\"0\",\"enable_id_trans\":\"0\",\"enable_duplicate_check\":\"0\",\"duplicate_check_interval\":\"1800\"}" --url "${notification_url}")"
   fi
   if [ "${notification_type}" ]; then
